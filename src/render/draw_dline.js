 ...
 * @private
 */
module.exports = function drawDLine(painter, sourceCache, layer, coords) {
  ...
  if (layer.paint['dline-width'] <= 0) return;

  const programId =
    layer.paint['dline-dasharray'] ? 'dlineSDF' :
      layer.paint['dline-pattern'] ? 'dlinePattern' : 'dline';
...
function drawLineTile(program, painter, tile, buffers, layer, coord, layerData, programChanged, tileRatioChanged) {
  const gl = painter.gl;
  const dasharray = layer.paint['dline-dasharray'];
  const image = layer.paint['dline-pattern'];
  ...
  if (programChanged || tileRatioChanged) {
    ...
    if (dasharray) {
      posA = painter.lineAtlas.getDash(dasharray.from, layer.layout['dline-cap'] === 'round');
      posB = painter.lineAtlas.getDash(dasharray.to, layer.layout['dline-cap'] === 'round');
  ...
  const posMatrix = painter.translatePosMatrix(coord.posMatrix, tile, layer.paint['dline-translate'], layer.paint['dline-translate-anchor']);
